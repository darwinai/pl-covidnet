...
from covidnet.data import process_image_file
...
class Inference():
    ...
    def infer(self):
        ...
        args = self.args
        args.imagepath = os.path.join(self.args.inputdir, self.args.imagefile)
    ...
    def generate_severity_data(self, imagePath):
        models = ['COVIDNet-SEV-GEO', 'COVIDNet-SEV-OPC']
        res = {}
        for modelName in models:
            args = {
                "weightspath": f"/usr/local/lib/covidnet/{modelName}",
                "metaname": "model.meta",
                "ckptname": "model",
                "input_size": 480,
                "top_percent": 0.08
            ...
            x = process_image_file(imagePath, args['top_percent'],
                                   args['input_size'])
            x = x.astype('float32') / 255.0
            ...
            elif modelName == 'COVIDNet-SEV-OPC':
    ...
    def generate_output_files(self, classification_data, severityScores):
        ...
        print("Copying over the input image to: {}...".format(
            self.args.outputdir))
        shutil.copy(self.args.inputdir + '/' + self.args.imagefile,
                    self.args.outputdir)
